...
(() => {
  ...
  async function putCached(key, response) {
    ...
    await transaction(DATA_STORE, "readwrite", (store) => store.put({ key, response: { body, status: response.status, headers: [...response.headers] }, updatedAt: Date.now() }));
  }
  function responseFromCache(cached) { return cached && new Response(cached.body, { status: cached.status, headers: cached.headers }); }
  async function listQueue() {
    return transaction(QUEUE_STORE, "readonly", (store) => new Promise((resolve) => {
  ...
  async function refreshPendingCount() {
    ...
    emit({ pending: items.length, conflicts: items.filter((item) => item.status === "conflict").length, authRequired: items.filter((item) => item.status === "auth-required").length });
    return items;
  }
  ...
  async function enqueue(request) {
    ...
    let body = request.method === "DELETE" ? "" : await request.clone().text();
    try {
      const payload = body ? JSON.parse(body) : {};
      payload.operationId = payload.operationId || operationId;
      body = JSON.stringify(payload);
    } catch (_) {}
    await transaction(QUEUE_STORE, "readwrite", (store) => store.put({ operationId, url: request.url, method: request.method, headers: [...request.headers], body, createdAt: Date.now(), attempts: 0 }));
  ...
  async function sync() {
    ...
    for (const item of items) {
      ...
      try {
        ...
        await updateOperation(item.operationId, { attempts: item.attempts + 1, lastError: error.message });
        break;
      }
    }
  ...
  window.fetch = async (input, init = {}) => {
    ...
    if (request.method === "GET") {
      try { const response = await nativeFetch(request); if (response.ok) await putCached(url.href, response); return response; }
      catch (error) { const cached = responseFromCache(await getCached(url.href)); if (cached) return cached; throw error; }
    }
    try { return await nativeFetch(request); } catch (_) { return enqueue(request); }
  };
  window.NooteOffline = { sync, getState: () => ({ ...state }), subscribe: (listener) => { listeners.add(listener); return () => listeners.delete(listener); }, clearUserCache: () => transaction(DATA_STORE, "readwrite", (store) => { const request = store.getAllKeys(); request.onsuccess = () => request.result.forEach((key) => store.delete(key)); }) };
